...
export default function AdminTools() {
  ...
  const handleAddTool = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewTool({ url: '', category_id: '', tags: '' });
    } catch (err) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-12 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <form onSubmit={handleAddTool} className="grid grid-cols-1 gap-4 md:grid-cols-4">
              ...
              <tr key={tool._id}>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    {tool.thumbnail && (
                      <img src={tool.thumbnail} className="h-10 w-10 rounded object-cover" />
                    )}
                    <div>
                      <div className="font-medium">{tool.title}</div>
                      <a href={tool.url} target="_blank" className="flex items-center gap-1 text-xs text-zinc-500 hover:text-blue-600">
